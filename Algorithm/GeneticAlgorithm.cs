...
namespace ArtificialIntelligenceIHW.Algorithm
{
        ...
        , ILongestPath
    {
        ...
        List<double> bestSolution = [ 0.0, 0.0 ];
        List<int> bestSolutionI = new();
        ...
        public void SolveLongestPathProblem(Graph g)
        {
            ...
            for (int i = 0; i < MaxIterations; i++)
            {
                ...
                while (newAgents.Count < AgentCount)
                {
                    ...
                    child.Fitness = PathLength(child.Values, g, 0.2);
        ...
        private void Mutate(Agent<int> agent, Graph g)
        {
            if (agent.Values.Count < 2 || rand.NextDouble() < MutationPossibility) return;
        ...
        private Agent<int> PathsCrossingover(Agent<int> p1, Agent<int> p2, Graph g)
        {
            ...
            if (minV < 2)
            {
                return new() { Values = new List<int>(p1.Values), Fitness = p1.Fitness };
            }
